...
module.exports = {
  ...
  process: function(msg) {
      ...
      msg.send({
        "attachments": [{
          ...
          "fields": [{
            ...
            "title": "사용법",
            "value": "mon pv { `user` | `mb` | `connect` } { `rps` }",
            "short": false
          }, {
            "title": "약어설명",
            ...
            "title": "설명",
            "value": "*mon pv user* \nuser api 응답시간을 ms단위로 보여준다\n\n*mon pv mb*\nmqttbroker api 응답시간을 ms단위로 보여준다\n\n*mon pv connect*\nconnect api 응답시간을 ms단위로 보여준다",
            "short": false
          }, {
            "title": "mon pv user rps",
            "value": "user api 초당 호출건수를 보여준다",
            "short": false
          }, {
            "title": "mon pv mb rps",
            "value": "mqttbroker api 초당 호출건수를 보여준다",
            "short": false
          }, {
            "title": "mon pv connect rps",
          ...
          "fields": [{
            ...
            "title": "사용법",
            "value": "mon pv { `user` | `mb` } { `post` | `get` | `put` | `delete` }",
            "short": false
          }, {
            "title": "사용예",
    ...
    } else if (arg[3] === 'user' | arg[3] === 'mb' | arg[3] === 'connect') {
      buildImage(msg);
    } else {
      sendMessage(msg.message.user.room, null, {
        "attachments": [{
          "pretext": "*에러발생*",
          "fields": [{
            //"title": "에러발생",
            "value": '잘못된 요청입니다 ' + msg.message.text,
            "short": false
          }],
          "mrkdwn_in": ["text", "pretext", "fields"],
          "color": "danger"
...
function getConnect() {
  ...
  api.get('/connect/apitestuser?token=0123456789')
    ...
    .end(function(err, res) {
      ...
      if (getconnectq.length > queueSize) {
        getconnectq.shift();
      }
      getconnectq.push(data);
      //log.debug('getuserq=' + util.inspect(getuserq));
      log.debug('getconnectq.length=' + getconnectq.length);
    });
}
  ...
  if (arg2) {
    if (arg2 === 'rps') {
      if (arg1 === 'user') {
        ...
        title = 'mon pv ' + arg1 + ' ' + arg2 + ' (request per second)';
        //legend: ['post', 'get', 'put', 'delete']
        result = all_html.replace(/legend:/g, "legend: ['get','post','put','del']");
        ...
        var cnt = connectReqCountq.length;
        log.debug('data.length=' + cnt);
        if (cnt < 1) {
          nodataFound();
          return;
      ...
      title = 'mov pv ' + arg1 + ' ' + arg2 + ' - elapsedtime(ms)';
    ...
    if (arg1 == 'user') {
      ...
      title = 'mon pv ' + arg1 + ' - elapsedtime(ms)';
      result = all_html.replace(/legend:/g, "legend: ['post', 'get', 'put', 'del']");
      result = result.replace(/realdata/g, JSON.stringify(data));
      result = result.replace(/title:/g, 'title: \'' + title + '\'');
      //legend: ['post', 'get', 'put', 'delete']
    } else if (arg1 == 'connect') {
      ...
      var cnt = postmqttbrokerq.length + getmqttbrokerq.length + putmqttbrokerq.length + delmqttbrokerq.length;
      log.debug('data.length=' + cnt);
      if (cnt < 1) {
        nodataFound();
        return;
