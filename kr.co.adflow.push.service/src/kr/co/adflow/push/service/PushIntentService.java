...
public class PushIntentService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (intent.getAction().equals("kr.co.adflow.push.service.START")) {
			...
			try {
				...
				Log.d(TAG, "token=" + token);
				// 처리후 결과 브로드캐스팅
				sendBroadcast(bundle.getString("serviceID"), msgID, token, true);
			} catch (Exception e) {
				Log.e(TAG, "에러발생", e);
				// 처리후 결과 브로드캐스팅
				sendBroadcast(bundle.getString("serviceID"), msgID,
			...
			try {
				...
				// 처리후 결과 브로드캐스팅
				sendBroadcast(bundle.getString("serviceID"), msgID, topic
						+ " 구독이완료되었습니다.", true);
			} catch (Exception e) {
				Log.e(TAG, "에러발생", e);
				// 처리후 결과 브로드캐스팅
	...
	private void sendBroadcast(String serviceID, String msgID, String content,
			boolean success) {
