...
public class MessageSendServiceImpl implements MessageSendService {
	...
	public int sendMessageArray(String serverId, int limit) {
		...
		for (Message msg : list) {
			...
			if (msg.getReceiverTopic() == null
				...
				logger.debug("============= msg.getReceiver()::"
						+ msg.getReceiver());
				msg.setReceiverTopic(this.getReceiverTopic(msg.getReceiver()));
				logger.debug("=============  msg.getReceiverTopic()"
	 ...
	 * 
	 * @see
	 * kr.co.adflow.pms.core.service.MessageSendService#sendReservationMessageArray
	...
	public void sendCallback(String serverId, int limit) {
		...
		for (AckCallback ack : list) {
			...
			if (ack.getCallbackUrl() != null
				...
				logger.debug("ack.getApplicationKey() {}",
				...
				if (ack.getCallbackMethod().equals("POST")) {
					try {
						...
						if (ack.getCallbackCountLimit() < 1
								|| ack.getCallbackCountLimit() <= ack
										.getCallbackCount()) {
							ack.setCallbackStatus(1);
						}
					} catch (Exception e) {
						logger.error(
								"callback Error. issueID::{} , CallbackUrl::{}",
								ack.getIssueId(), ack.getCallbackUrl());
						e.printStackTrace();
						// error
