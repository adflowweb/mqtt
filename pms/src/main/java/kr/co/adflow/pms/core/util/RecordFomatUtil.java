...
public class RecordFomatUtil {
	public static String intFormat(int valueInt, int stringLen) {
		...
		try {
			...
			if (stringLen < valueLen) {
				// new
				// TokenRuntimeException("String Length is smail-StringLen:"+stringLen+", valueInt:"+valueInt);
			}
	...
	public static String stingFormat(String value, int stringLen) {
		...
		try {
			...
			// length check
			if (stringLen < valueLen) {
				// new TokenRuntimeException("String Length is smail-StringLen:"
	...
	public static String topicToUfmiNo(String topic) {
		...
		try {
			...
			if (pTalkVer.equals("1")) {
				...
				// 국번
				sb.append(RecordFomatUtil.intFormat(Integer.parseInt(topicTemp
						.substring(firstT, lastT).replace("/", "")), 4));
				// 개별번호
				sb.append(RecordFomatUtil.intFormat(
						Integer.parseInt(topicTemp.substring(lastT,
								topicTemp.length()).replace("/", "")), 5));
			} else if (pTalkVer.equals("2")) {
				// P-tail 2.0
				// 01~04
				sb.append(RecordFomatUtil.intFormat(
						Integer.parseInt(topicTemp.substring(0, firstT)), 2));
				// 국번
				sb.append(RecordFomatUtil.intFormat(Integer.parseInt(topicTemp
						.substring(firstT, lastT).replace("/", "")), 4));
				// 개별번호
				sb.append(RecordFomatUtil.intFormat(
						Integer.parseInt(topicTemp.substring(lastT,
								topicTemp.length()).replace("/", "")), 4));
			} else {
				// new TokenRuntimeException("ufmi Topic not valid - Topic:"
				// + topic);
			}
	...
	public static String ufmiNo(String ufmi) {
		int valueLen;
		StringBuffer sb = new StringBuffer();
