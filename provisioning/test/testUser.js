...
describe('user', function() {
    ...
    it('요청 API 버전이 다른경우 response 400, code: \'InvalidVersion\' 을 반환 해야한다', function(done) {
      api.post('/user/01040269329')
        ...
        .end(function(err, res) {
          ...
          expect(res.body.message).to.equal('2.0.0 is not supported by POST /user/01040269329');
          done();
        });
    });
  ...
  describe('조회', function() {
    it('결과에 토큰, 전화번호, mqttbroker 속성이 존재 해야한다', function(done) {
      api.get('/user/01040269329?token=0123456789')
        .set('Content-Type', 'application/json')
        .set('Accept-Version', '1.0.0')
        // .send({
        //   token: '0123456789',
    ...
    it('요청 API 버전이 다른경우 response 400, code: \'InvalidVersion\' 을 반환 해야한다', function(done) {
      api.get('/user/01040269329?token=0123456789')
        ...
        .end(function(err, res) {
          ...
          //{ code: 'InvalidVersion', message: '2.0.0 is not supported by POST /user/test' }
          expect(res.body.code).to.equal('InvalidVersion');
          expect(res.body.message).to.equal('2.0.0 is not supported by GET /user/01040269329');
          done();
        });
    });
    ...
    it('요청시 해당사용자가 없는 경우 response 404, code: \'ResourceNotFound\' 을 반환 해야한다', function(done) {
      //var max = Math.max(1, 2, 10, 3);
      //expect(max).to.equal(10);
      api.get('/user/nonexistUser?token=0123456789')
        .set('Content-Type', 'application/json')
        .set('Accept-Version', '1.0.0')
        // .send({
        //   token: '0123456789',
    ...
    it('요청 API 버전이 다른경우 response 400, code: \'InvalidVersion\' 을 반환 해야한다', function(done) {
      api.put('/user/01040269329')
        ...
        .end(function(err, res) {
          ...
          //{ code: 'InvalidVersion', message: '2.0.0 is not supported by POST /user/test' }
          expect(res.body.code).to.equal('InvalidVersion');
          expect(res.body.message).to.equal('2.0.0 is not supported by PUT /user/01040269329');
          done();
        });
    });

    it('요청시 필수항목(token||phone||mqttbroker)이 없는 경우 response 409, code: \'InvalidArgument\' 을 반환 해야한다', function(done) {
      //var max = Math.max(1, 2, 10, 3);
      //expect(max).to.equal(10);
      api.put('/user/01040269329')
        .set('Content-Type', 'application/json')
  ...
  describe('삭제', function() {
    it('요청을 수행하고 response 200 을 반환 해야한다', function(done) {
      api.del('/user/01040269329?token=0123456789')
        .set('Content-Type', 'application/json')
        .set('Accept-Version', '1.0.0')
        // .send({
        //   token: '0123456789',
    ...
    it('요청 API 버전이 다른경우 response 400, code: \'InvalidVersion\' 을 반환 해야한다', function(done) {
      api.del('/user/01040269329')
        ...
        .end(function(err, res) {
          ...
          //{ code: 'InvalidVersion', message: '2.0.0 is not supported by POST /user/test' }
          expect(res.body.code).to.equal('InvalidVersion');
          expect(res.body.message).to.equal('2.0.0 is not supported by DELETE /user/01040269329');
          done();
        });
    });

    it('요청시 해당사용자가 없는 경우 response 404, code: \'ResourceNotFound\' 을 반환 해야한다', function(done) {
      api.del('/user/nonexistUser?token=0123456789')
        .set('Content-Type', 'application/json')
        .set('Accept-Version', '1.0.0')
        // .send({
        //   token: '0123456789',
